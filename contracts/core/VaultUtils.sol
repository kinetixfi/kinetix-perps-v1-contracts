...
contract VaultUtils is IVaultUtils, Governable {
    ...
    function getBuyUsdkFeeBasisPoints(address _token, uint256 _usdkAmount) public override view returns (uint256) {
        return getFeeBasisPoints(_token, _usdkAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), true);
    }

    function getSellUsdkFeeBasisPoints(address _token, uint256 _usdkAmount) public override view returns (uint256) {
        return getFeeBasisPoints(_token, _usdkAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), false);
    }

    function getSwapFeeBasisPoints(address _tokenIn, address _tokenOut, uint256 _usdkAmount) public override view returns (uint256) {
    ...
    function getFeeBasisPoints(address _token, uint256 _usdkDelta, uint256 _feeBasisPoints, uint256 _taxBasisPoints, bool _increment) public override view returns (uint256) {
    ...
    function getMaxAmountIn(
        ...
        uint256 maxUsdkAmount = vault.maxUsdkAmounts(_tokenIn);

        if (maxUsdkAmount != 0) {
            if (maxUsdkAmount < vault.usdkAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsdkAmount.sub(vault.usdkAmounts(_tokenIn));
